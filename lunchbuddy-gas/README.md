# 🍱 오늘점심 — 앱스크립트 배포 가이드 (⚠️ 더 이상 운영에 사용 안 함)

> **2026-07-09 Supabase로 이전 완료.** 이 폴더는 참고용으로만 남겨두었습니다.
> 현재 운영 중인 백엔드는 [`supabase/`](../supabase/) 폴더의 SQL을 사용합니다. 새로 배포하려면 루트 [README.md](../README.md)를 따르세요.

구글 시트(데이터) + 구글 앱스크립트(JSON API 백엔드) + GitHub Pages(프론트엔드)로 동작하던
사내 점심 멤버 확인 서비스의 이전 버전입니다. (요구사항 명세서 v1.3 기준)
...
### 이메일 알림 (REQ-14)
- 회원가입/설정에서 이메일을 **선택 입력**한 사람에게만 발송됩니다 (알림 발송 목적으로만 사용, 설정에서 즉시 삭제 가능)
- 호스트·공동 호스트가 **설정 > 알림 설정**에서 방별로 발송 여부·발송 시각을 지정합니다
- **평일(월~금)** 에만, 설정 시각 전후로 하루 1회 발송됩니다 (5분 간격 트리거가 확인 — 설정 시각보다 최대 5분+α 늦게 발송될 수 있음)
- 수신자 주소는 **BCC**로 발송되어 서로에게 노출되지 않습니다
- ⚠️ 발송 계정은 **`setup()`을 실행·승인한 계정(yujeong.dk)** 이며 **무료 한도는 1일 100통**입니다 (방·인원이 많으면 초과 주의). 코드 교체 후 `setup()` 재실행도 같은 계정으로 해야 발송 계정이 유지됩니다
- ⚠️ 코드 교체 후 `setup()` 재실행 시 **이메일 발송 권한 승인**이 추가로 필요할 수 있습니다 (권한 창에서 허용)
...
| 채팅 | 메인 화면 내 섹션, 텍스트 300자, **오늘 메시지만 표시·매일 자정 리셋**, 15초 폴링 |
| 이메일 알림 | 선택 수집, 호스트가 방별 발송 여부·시각 설정, 평일만 발송, 5분 트리거 (하루 1회, BCC) |
| 데이터 보관 | 히스토리 30일 보관 / 채팅 당일만 (매일 리셋) 후 자동 삭제 |
